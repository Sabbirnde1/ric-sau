...
interface InnovatorsClientProps {
  initialInnovations: any[];
}

export default function InnovatorsClient({ initialInnovations }: InnovatorsClientProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState("All");
  const innovations = initialInnovations.length > 0 ? initialInnovations : defaultInnovations;
    ...
    <div ref={containerRef} className="min-h-screen bg-background pt-20">
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 text-sm rounded-full border transition ${
                  filter === cat ? "bg-primary text-white border-primary" : "border-gray-300 hover:bg-gray-100"
                }`}
              >
                {cat}
              </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredInnovations.map((item: any, i: number) => (
            ...
            >
              {item.ripd && <div className="text-xs font-semibold text-primary mb-2">RIPD: {item.ripd}</div>}
      ...
      <section className="py-24 bg-gradient-to-r from-indigo-700 via-blue-600 to-teal-500 text-white text-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl lg:text-4xl font-bold mb-6">Collaborate with RIC-SAU</h2>
          <p className="text-lg mb-8 max-w-2xl mx-auto text-blue-100">
            Partner with us to turn innovative ideas into impactful real-world solutions.
          </p>
          <Link href="/contact">
