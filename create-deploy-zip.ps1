...
Write-Host "`n[INFO] Project directory: $projectRoot" -ForegroundColor Yellow
...
Write-Host "`n[INFO] Scanning project files..." -ForegroundColor Yellow
...
Write-Host "`n[OK] Found $fileCount files to include" -ForegroundColor Green
Write-Host "[INFO] Total size: $sizeMB MB" -ForegroundColor Green

if ($sizeMB -gt 50) {
    Write-Host "`n[WARNING] ZIP size is larger than expected!" -ForegroundColor Red
...
Write-Host "`n[INFO] Creating ZIP file..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "   For detailed instructions, see: NETLIFY_DEPLOYMENT.md" -ForegroundColor White
    
    # Ask if user wants to open the folder
    $openFolder = Read-Host "`nOpen folder containing ZIP file? (Y/n)"
